...
void cache_init(int blockSize, int numSets, int blocksPerSet)
{
    ...
    cache.num_hit = 0;
    cache.num_miss = 0;
    cache.num_writeback = 0;

    int total_block = numSets * blocksPerSet;
...
int cache_access(int addr, int write_flag, int write_data)
{
    ...
    //     fully_associative = 1;
    // }
    // if (cache.blocksPerSet == 1) {
    //     direct_mapped = 1;
    // }
    // int total_cache_blocks = cache.numSets * cache.blocksPerSet;
    // int num_bits_tag = 16 - (int)log2(cache.blockSize) - (int)log2(cache.numSets);
    // int tar_tag = addr >> (16 - num_bits_tag);
    // int offset = addr << (16 - (int)log2(cache.blockSize));
    // int value;
    // if (fully_associative) {
    //     if (write_flag == 0) { // read
    //         for (int i = 0; i < total_cache_blocks; i++) {
    //             if (cache.blocks[i].valid && cache.blocks[i].tag == tar_tag) {
    //                 value = cache.blocks[i].data[offset];
    //             }
    //         }
    //     }
    //     else {
    ...
    if (hit != -1) {
        ...
        for (int i = 0; i < cache.blocksPerSet; i++) {
            ...
            else if (cache.blocks[index].valid) {
                cache.blocks[index].lruLabel++;
            }
        }
        
        if (write_flag) {   
            cache.blocks[hit].data[offset] = write_data;
            cache.blocks[hit].dirty = 1;
            printAction(addr, 1, processorToCache);
            return 0;   
        } 
        else {        
            int data = cache.blocks[hit].data[offset];
    ...
    if (victim == -1) {
        ...
        for (int i = 0; i < blocksPerSet; i++) {
            int index = set_start + i;
            if (cache.blocks[index].valid && cache.blocks[index].lruLabel > max_lru) {
                max_lru = cache.blocks[index].lruLabel;
                victim = index;
            }
        }
    }
    
    if (cache.blocks[victim].valid && cache.blocks[victim].dirty) {
        int victim_block_num = cache.blocks[victim].tag * numSets + set_index;
        int victim_base_addr = victim_block_num * blockSize;
        for (int i = 0; i < blockSize; i++) {
            mem_access(victim_base_addr + i, 1, cache.blocks[victim].data[i]);
        }
        printAction(victim_base_addr, blockSize, cacheToMemory);
        cache.num_writeback++;
    } 
    else if (cache.blocks[victim].valid) {
        int victim_block_num = cache.blocks[victim].tag * numSets + set_index;
        int victim_base_addr = victim_block_num * blockSize;
        printAction(victim_base_addr, blockSize, cacheToNowhere);
    }
    
    int new_base_addr = (addr / blockSize) * blockSize;
    for (int i = 0; i < blockSize; i++) {
        cache.blocks[victim].data[i] = mem_access(new_base_addr + i, 0, 0);
    }
    printAction(new_base_addr, blockSize, memoryToCache);
    
    cache.blocks[victim].tag = tag;
    cache.blocks[victim].valid = 1;
    cache.blocks[victim].dirty = 0;
    
    if (write_flag) {   
        cache.blocks[victim].data[offset] = write_data;
        cache.blocks[victim].dirty = 1;
        printAction(addr, 1, processorToCache);
        int start = set_index * cache.blocksPerSet;
        for (int i = 0; i < cache.blocksPerSet; i++) {
            int index = start + i;
        ...
        int data = cache.blocks[victim].data[offset];
        printAction(addr, 1, cacheToProcessor);
        int start = set_index * cache.blocksPerSet;
        for (int i = 0; i < cache.blocksPerSet; i++) {
            int index = start + i;
...
void printStats(void)
{
    ...
    // return;
    printf("End of run statistics:\n");
    printf("hits %d, misses %d, writebacks %d\n", cache.num_hit, cache.num_miss, cache.num_writeback);
    
    int dirty_blocks = 0;
    int total_blocks = cache.numSets * cache.blocksPerSet;
